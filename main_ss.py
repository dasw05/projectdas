...
def save_results(first_eval,tr_hist,num_classes,sub,split,n_ds,n_ch,T):
    ...
    # Save metrics  
    results = np.zeros((4,1+len(tr_hist.history['accuracy'])))
    ...
    # retraing results
    results[0,1:] = tr_hist.history['accuracy']
    results[1,1:] = tr_hist.history['val_accuracy']
    results[2,1:] = tr_hist.history['loss']
    results[3,1:] = tr_hist.history['val_loss']

    sub_str = '{0:03d}'.format(sub)
    results_str = '/content/results/your-global-experiment/your-global-experiment/stats/ss_class_{num_classes}_ds{n_ds}_nch{n_ch}_T{T}_sub{sub_str}_split_{split}.csv'
...
global_model_path = '/content/results/your-global-experiment/your-global-experiment/model/'
